...
class TestBuildGenerationPlan:
    ...
    def test_raises_error_for_empty_formats(
        self, tmp_path: Path, story: Scenario
    ) -> None:
        story.given("generation options with no formats specified")
        options = GeneratePlanOptions(
            name="candidate",
            data_dir=tmp_path,
            template=None,
        ...
        story.when("the plan builder runs with no formats")
        with pytest.raises(ValueError, match="At least one output format"):
            build_generation_plan(options)

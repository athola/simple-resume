...
class TestCVBusinessRules:
    ...
    def test_user_story_professional_cv_creation(self, temp_dir: Path):
...
- **Team productivity**: 3x increase in deployment frequency
        ...
        processed_cv = self._process_cv_with_business_logic(professional_cv_data, temp_dir)

        # Acceptance Criteria 3: Business rules for professional CVs
        self._validate_professional_cv_business_rules(processed_cv)

    def test_user_story_student_cv_creation(self, temp_dir: Path):
...
- **Open source**: 50+ stars on GitHub
    ...
    def test_user_story_career_change_cv(self, temp_dir: Path):
...
- **Problem Solving**: Analytical mindset applied to technical challenges
        ...
        processed_cv = self._process_cv_with_business_logic(career_change_cv_data, temp_dir)
        self._validate_career_change_cv_business_rules(processed_cv)

    def test_business_rule_cv_length_constraints(self, temp_dir: Path):
        ...
        processed_appropriate = self._process_cv_with_business_logic(appropriate_cv, temp_dir)
        processed_long = self._process_cv_with_business_logic(long_cv, temp_dir)

        # Assert business rules about CV length
        assert self._get_cv_content_length(processed_short) < 100, "Short CV should be flagged as insufficient"
        assert 100 <= self._get_cv_content_length(processed_appropriate) <= 5000, "Appropriate CV should pass length validation"
        assert self._get_cv_content_length(processed_long) > 5000, "Long CV should be flagged as excessive"

    def test_business_rule_contact_information_validation(self, temp_dir: Path):
        ...
        for test_case in test_cases:
            processed_cv = self._process_cv_with_business_logic(test_case["data"], temp_dir)

            if test_case["should_be_valid"]:
                assert self._validate_contact_information(processed_cv), f"Contact validation failed for {test_case['name']}"
            else:
                assert not self._validate_contact_information(processed_cv), f"Contact validation should fail for {test_case['name']}"

    def test_business_rule_template_appropriateness(self, temp_dir: Path):
        ...
        for scenario in test_scenarios:
            processed_cv = self._process_cv_with_business_logic(scenario["data"], temp_dir)
    ...
    def _validate_professional_cv_business_rules(self, cv_data: Dict[str, Any]) -> None:
        ...
        # Email validation
        assert "@" in cv_data["email"] and "." in cv_data["email"], "Email must be valid"

        # Phone validation
        phone = cv_data["phone"].replace(" ", "").replace("-", "").replace("(", "").replace(")", "")
        assert phone.replace("+", "").isdigit(), "Phone must contain only digits and standard characters"

        # Experience validation
        assert "Experience" in cv_data["body"], "Professional CV must have experience section"
        assert len(cv_data["body"]["Experience"]) > 0, "Professional CV must have at least one experience entry"
    ...
    def _validate_student_cv_business_rules(self, cv_data: Dict[str, Any]) -> None:
        ...
        if "Education" in cv_data.get("body", {}):
            ...
            assert len(education_entries) > 0, "Student CV should have education entries"

        # Should have projects or extracurricular activities
        body_sections = cv_data.get("body", {})
        has_projects = "Projects" in body_sections and len(body_sections["Projects"]) > 0
        has_experience = "Experience" in body_sections and len(body_sections["Experience"]) > 0

        assert has_projects or has_experience, "Student CV should have projects or relevant experience"

    def _validate_career_change_cv_business_rules(self, cv_data: Dict[str, Any]) -> None:
        ...
        transition_keywords = ["transition", "career", "change", "pivot", "new", "learning"]
        has_transition_narrative = any(keyword.lower() in description.lower() for keyword in transition_keywords)

        assert has_transition_narrative, "Career change CV should explain the transition"
        ...
        assert has_certifications or has_education, "Career change CV should show new qualifications"

    def _validate_contact_information(self, cv_data: Dict[str, Any]) -> bool:
        """Validate contact information format and completeness."""
        contact_methods = 0

        if "email" in cv_data and cv_data["email"]:
            email = cv_data["email"]
        ...
        if "web" in cv_data and cv_data["web"]:
            if cv_data["web"].startswith(("http://", "https://")):
                contact_methods += 1

        if "linkedin" in cv_data and cv_data["linkedin"]:
            if "linkedin.com" in cv_data["linkedin"] or cv_data["linkedin"].startswith("in/"):
                contact_methods += 1
    ...
    def _process_cv_with_business_logic(self, cv_data: Dict[str, Any], temp_dir: Path) -> Dict[str, Any]:
